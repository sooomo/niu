...
func hubKey(userId string, platform core.Platform) string {
...
func (c *Line) Platform() core.Platform { return c.platform }
...
func NewHub(
	...
	err = h.pool.Submit(func() {
		for conn := range h.registeredChan {
			conn.closeChan = make(chan core.Empty)
...
func (h *Hub) Close(wait time.Duration) {
	...
	h.connections.Range(func(key, value any) bool {
		value.(*Line).closeChan <- core.Empty{}
		return true
	})
...
func (h *Hub) CloseLine(userId string, platform core.Platform) {
	if len(userId) == 0 {
		return
	}

	if platform == core.Unspecify {
		for _, p := range core.Platforms {
			conn, ok := h.connections.Load(hubKey(userId, p))
			if !ok {
				continue
			}
			conn.(*Line).closeChan <- core.Empty{}
		}
	} else {
...
func (h *Hub) CloseLineExcept(userId string, exceptPlatform core.Platform) {
	if len(userId) == 0 {
		return
	}
...
func (h *Hub) UpgradeWebSocket(userId string, platform core.Platform, w http.ResponseWriter, r *http.Request) error {
	conn, err := h.upgrader.Upgrade(w, r, nil)
	if err != nil {
		return err
	}

	existCon, ok := h.connections.Load(hubKey(userId, core.Platform(platform)))
	if ok {
		// 已经存在该平台的连接，关闭该连接
		existCon.(*Line).closeChan <- core.Empty{}
	}
	// 存下该平台新的连接
	cc := &Line{
		conn:       conn,
